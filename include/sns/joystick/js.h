...
struct js_event {
    ...
    uint8_t number;  ///< Affected joystick actuator (i.e., which axis moved,
 ...
 */
struct js_state {
    uint8_t buttons[JS_BUTTON_CNT];  ///< State of all joystick buttons (note:
                                     ///< most joysticks have less than 256
                                     ///< buttons). Can be treated as booleans
                                     ///< where True is pressed
    double axes[JS_AXIS_CNT];  ///< Position of all joystick axes. These values
                               ///< are clamped between -1.0 and 1.0
};
 ...
 *
 * @param index The index of the joystick to open. This will try to directly
 * open the Linux input device with the associated index
 ...
 * event and does not update the joystick's state. Additionally, multiple calls
 * to this function will overwrite the static js_event_t.
 *
